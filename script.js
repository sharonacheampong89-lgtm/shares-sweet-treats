...
{cat:'Brownies & Bars',name:'Classic Fudge',price:3.25,desc:'Rich, fudgy brownie.',badge:'Fudgy'},{cat:'Brownies & Bars',name:'Nutella Swirl',price:3.25,desc:'Brownie with chocolate hazelnut swirl.',badge:'Swirl'},{cat:'Brownies & Bars',name:'Biscoff Brownies',price:3.25,desc:'Brownies with Biscoff flavor.',badge:'Popular'},{cat:'Brownies & Bars',name:'Oreo Cheesecake Brownies',price:3.25,desc:'Oreo cheesecake brownie bar.',badge:'Creamy'},{cat:'Brownies & Bars',name:'Turtle Brownies',price:3.25,desc:'Chocolate, caramel and pecan style brownie.',badge:'Caramel'},
...
function init(){document.getElementById('categoryTabs').innerHTML=cats.map(c=>`<button class="${c===activeCat?'active':''}" onclick="setCat('${c}')">${c}</button>`).join('');renderMenu();renderCart();renderAdmin();setMinDate();}
...
function deliveryFee(){let type=document.getElementById('orderType')?.value; let m=Number(document.getElementById('miles')?.value||0); if(type!=='Local Delivery')return 0; if(m<=5)return 5;if(m<=10)return 8;if(m<=15)return 10;if(m<=20)return 15;return 0}
