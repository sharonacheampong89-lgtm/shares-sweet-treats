...
function metadataAddress(session) {
  const md = session.metadata || {};
  const cityLine = [md.city, md.state, md.zip].filter(Boolean).join(' ');
  return [md.street, md.apt, cityLine].filter(Boolean).join(', ') || md.address || '';
}
...
function buildShippingText(session) {
  ...
  const fallback = metadataAddress(session);
  if (fallback) return fallback;

  return 'No shipping/delivery address collected.';
}
...
async function sendOwnerSms({ customerName, customerPhone, orderType, preferredDate, preferredTime, total, items, sessionId, address }) {
...
async function saveOrderToSupabase(session, items) {
  ...
  const deliveryFee = Number(session.metadata?.delivery_fee || session.metadata?.service_fee || 0);
  ...
  const deliveryDistance = Number(session.metadata?.delivery_distance || session.metadata?.delivery_miles || 0);
  ...
  const payload = {
    ...
    tip,
    subtotal: subtotal ? dollars(subtotal) : null,
    total,
...
module.exports = async function handler(req, res) {
  try {
    ...
    const orderAddress = buildShippingText(session);
    ...
    const smsResult = await sendOwnerSms({ customerName, customerPhone, orderType, preferredDate, preferredTime, total, items, sessionId: orderId, address: orderAddress });
