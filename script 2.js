...
function updateDeliveryFields(){
  ...
  updateSchedulingFields(type);

  calculatedDelivery={fee:0,miles:null,available:false,address:''};
  if(type==='Local Delivery'){
    estimator.innerHTML='Enter the full delivery address, then click <button type="button" class="btn secondary" onclick="calculateDeliveryFee(true)">Calculate Delivery Fee</button><br><small>Delivery is available within 20 miles of the North Charleston area. Your exact pickup location is not shown publicly.</small>';
  }else if(type==='Mail Shipping'){
    estimator.innerHTML='<b>Mail Shipping Notice:</b> Shipping orders do not require a pickup/delivery date or time. We will email you when your order is ready to ship and when it has been delivered.<br><small>Flat shipping fee: <b>$12.00</b>. Cheesecake items cannot be shipped.</small>';
  }else{
    estimator.innerHTML='Pickup location and instructions will be provided after your order is confirmed.';
  }
  renderCart();
}

function updateSchedulingFields(type){
  const shouldHide=type==='Mail Shipping';
  ['date','time','preferred_date','preferred_time'].forEach(name=>{
    const input=document.querySelector(`[name="${name}"]`);
    if(!input) return;
    input.disabled=shouldHide;
    input.required=!shouldHide;
    if(shouldHide) input.value='';
    const wrapper=input.closest('.form-group') || input.closest('label') || input.parentElement;
    if(wrapper) wrapper.classList.toggle('hidden', shouldHide);
  });

  let scheduleNotice=document.getElementById('shippingScheduleNotice');
  const form=document.querySelector('form');
  if(!scheduleNotice && form){
    scheduleNotice=document.createElement('p');
    scheduleNotice.id='shippingScheduleNotice';
    scheduleNotice.className='note hidden';
    scheduleNotice.innerHTML='📦 Shipping orders do not need a selected day or time. You will receive email updates when your order is ready to ship and when it has been delivered.';
    const deliveryFields=document.getElementById('deliveryFields');
    if(deliveryFields) deliveryFields.appendChild(scheduleNotice);
    else form.appendChild(scheduleNotice);
  }
  if(scheduleNotice) scheduleNotice.classList.toggle('hidden', !shouldHide);
}
