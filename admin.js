...
async function loadOrders(){
  ...
  try{
    ...
    list.innerHTML = `<p class="error">${clean(err.message)}</p>`;
  }
}
...
async function loadInventory(){
  ...
  try{
    ...
    if(list) list.innerHTML = `<p class="error">${clean(err.message)}</p>`;
  }
}
...
function itemList(items){
  ...
  return arr.map(i => {
    const qty = clean(i.qty || i.quantity || 1);
    const name = clean(i.name || 'Menu item');
    const price = i.price ? ' - ' + money(i.price) : '';
...
function orderAddress(o){
  const address = clean(o.delivery_address || o.address || '');
  if(!address || address.toLowerCase().includes('no shipping')) return 'No shipping/delivery address collected.';
  return address;
}
...
<strong>Address:</strong> ${orderAddress(o)}
</div>
<div class="box">
<h3>Items</h3>
<ul>${(Array.isArray(o.items)?o.items:[]).map(i=>`<li><span class="check"></span>${clean(i.qty || i.quantity || 1)} x ${clean(i.name)}</li>`).join('') || '<li>No items listed</li>'}</ul>
</div>
<div class="box">
<h3>Notes</h3>
  ...
  return `<article class="order-card">
    <div class="order-top">
      <div>
        ...
        <h3>${orderNumber(o)} â€¢ ${clean(o.customer_name || 'Customer')} - ${money(o.total)}</h3>
        <div class="meta">
    ...
    <div class="actions">
      ...
      ${!isShippingOrder(o) ? `<button class="small" onclick="editOrderAddress('${o.id}')">Edit Address</button>` : ''}
